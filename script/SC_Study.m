...
cd(S.sDataDir);
matName = ['SC_study_sub' num2str(S.sNum), '_date_' S.sName 'out.mat'];
checkEmpty = isempty(dir (matName));
suffix = 1;
while checkEmpty ~=1
    suffix = suffix+1;
    matName = ['SC_study_sub' num2str(S.sNum), '_' S.sName 'out(' num2str(suffix) ').mat'];
    checkEmpty = isempty(dir (matName));
end
